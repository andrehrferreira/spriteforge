...
export function initEditor(anim: AnimationData, frames: ExtractedFrame[]): () => void {
    ...
    nameInput.oninput = () => {
        anim.name = nameInput.value.trim() || anim.name
    }
    ...
    document.querySelectorAll<HTMLButtonElement>('.bg-chip').forEach((btn) => {
        ...
        btn.onclick = () => {
            ...
            previewWrap.className =
                'preview-wrap ' + (btn.dataset.bg === 'checker' ? 'checker' : 'bg-' + btn.dataset.bg)
        }
    })
    ...
    colorInput.oninput = () => {
        ...
        settings.key = [parseInt(v.slice(1, 3), 16), parseInt(v.slice(3, 5), 16), parseInt(v.slice(5, 7), 16)]
        syncSwatch()
        applyChroma()
    }
    ...
    bindSlider('#sim-range', '#sim-val', settings.similarity, (v) => {
        settings.similarity = v
    })
    bindSlider('#smooth-range', '#smooth-val', settings.smoothness, (v) => {
        settings.smoothness = v
    })
    bindSlider('#spill-range', '#spill-val', settings.spill, (v) => {
        settings.spill = v
    })
    bindSlider('#halo-range', '#halo-val', settings.halo, (v) => {
        settings.halo = v
    })
    ...
    document.querySelectorAll<HTMLButtonElement>('[data-curve]').forEach((btn) => {
        btn.onclick = () => {
            if (btn.dataset.curve === 'in')
                anim.curve = [
                    { t: 0, v: 0.5 },
                    { t: 1, v: 2 },
                ]
            else if (btn.dataset.curve === 'out')
                anim.curve = [
                    { t: 0, v: 2 },
                    { t: 1, v: 0.5 },
                ]
    ...
    $('#btn-all').onclick = () => {
        anim.selected = anim.selected.map(() => true)
        anim.drift = null
        onSelectionChange()
    }
    $('#btn-none').onclick = () => {
        anim.selected = anim.selected.map(() => false)
        anim.drift = null
        onSelectionChange()
    }
    $('#btn-invert').onclick = () => {
        anim.selected = anim.selected.map((s) => !s)
        anim.drift = null
        onSelectionChange()
    }
