...
export function detectContinuity(inputs: AnimInput[], cfg: SpritesCfg): Correction[] {
    ...
    for (const i of inputs) {
        ...
        for (let p = 1; p < n; p++)
            internal.push(diffPct(i.samples[p - 1], i.samples[p], cfg.pixelDiffThreshold))
        ...
        if (seam > cfg.continuityThresholdPct && seam > typical * 2) {
            out.push(
                corr(
                    'continuidade',
                    'aviso',
                    { scope: 'frame', animId: i.id, frames: [n - 1] },
