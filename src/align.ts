...
export function initAlign(): () => void {
    ...
    async function prepare(recomputeOnly = false): Promise<void> {
        ...
        for (let i = 0; i < anims.length; i++) {
            ...
            try {
                ...
                loops.set(
                    a.id,
                    createLoop(proc, sel, a.chroma, a.crossfade, contiguous ? (a.drift ?? null) : null),
                )
    ...
    function drawAnim(
        ...
        pos: number,
        alpha: number,
        cell: ReturnType<typeof cellDims>,
        ox: number,
        oy: number,
        vs: number,
    ): void {
    ...
    function renderAlign(): void {
        ...
        ctx.beginPath()
        ctx.moveTo(pxv - 14, pyv)
        ctx.lineTo(pxv + 14, pyv)
        ctx.moveTo(pxv, pyv - 14)
        ctx.lineTo(pxv, pyv + 14)
        ctx.stroke()
    }
    ...
    function buildTabs(): void {
        ...
        ghostSel.innerHTML =
            '<option value="">sem ghost</option>' +
            project.animations
                .filter((a) => loops.has(a.id))
                .map(
                    (a) =>
                        `<option value="${a.id}"${a.id === ghostId ? ' selected' : ''}>ghost: ${a.name}</option>`,
                )
                .join('')
        ghostSel.onchange = () => {
    ...
    function buildRows(): void {
        ...
        for (const a of project.animations) {
            ...
            name.title = bounds.get(a.id)
                ? 'clique para ver no preview'
                : 'sem conteúdo visível após o chroma'
            name.onclick = () => {
                currentId = a.id
                loopPos = 0
                buildTabs()
            ...
            row.append(name)
            row.append(
                miniInput('X', a.align.dx, 1, (v) => {
                    a.align.dx = v
                }),
            )
            row.append(
                miniInput('Y', a.align.dy, 1, (v) => {
                    a.align.dy = v
                }),
            )
            row.append(
                miniInput('ESC', a.align.scale, 0.05, (v) => {
                    a.align.scale = Math.max(0.05, v)
                }),
            )
            rows.append(row)
        }
    ...
    pivotX.oninput = () => {
        cfg.pivotX = clamp01(Number(pivotX.value))
        afterCfgChange()
    }
    pivotY.oninput = () => {
        cfg.pivotY = clamp01(Number(pivotY.value))
        afterCfgChange()
    }
    marginInput.oninput = () => {
