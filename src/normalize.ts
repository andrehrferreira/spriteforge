...
export function initNormalize(): () => void {
  ...
  bindSlider('#n-tol', '#n-tol-val', removal.tol, (v) => { removal.tol = v })
  bindSlider('#n-feather', '#n-feather-val', removal.feather * 10, (v) => { removal.feather = v / 10 })
  bindSlider('#n-sim', '#n-sim-val', Math.round(settings.similarity * 1000), (v) => { settings.similarity = v / 1000 })
  bindSlider('#n-smooth', '#n-smooth-val', Math.round(settings.smoothness * 1000), (v) => { settings.smoothness = v / 1000 })
  ...
  function buildRows(): void {
    ...
    for (const img of images) {
      ...
      row.append(name)
      row.append(miniInput('Y', img.dy, 1, (v) => { img.dy = v }))
      row.append(miniInput('ESC', img.scale, 0.05, (v) => { img.scale = Math.max(0.05, v) }))
  ...
  function compose(target: CanvasRenderingContext2D, img: RefImage, vs: number, ox: number, oy: number): void {
    const b = img.bounds
    if (!b) return
    const s = img.scale
    const ax = b.x + b.w / 2
