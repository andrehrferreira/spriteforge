    ...
    return boxes.map((b) => {
        ...
        for (const o of boxes) {
            ...
            if (vOverlap) {
                ...
                if (o.x >= b.x + b.w)
                    right = Math.min(right, Math.max(0, Math.floor((o.x - (b.x + b.w)) / 2)))
            }
            if (hOverlap) {
                if (o.y + o.h <= b.y) top = Math.min(top, Math.max(0, Math.floor((b.y - (o.y + o.h)) / 2)))
                if (o.y >= b.y + b.h)
                    bottom = Math.min(bottom, Math.max(0, Math.floor((o.y - (b.y + b.h)) / 2)))
            }
        }
...
export function initSlicer(): () => void {
    ...
    function render(): void {
        ...
        boxes.forEach((b, i) => {
            ...
            if (sel) {
                ...
                ctx.fillRect(
                    x + bw - 6 * devicePixelRatio,
                    y + bh - 6 * devicePixelRatio,
                    6 * devicePixelRatio,
                    6 * devicePixelRatio,
                )
            }
    ...
    function syncSelInputs(): void {
        const b = selected >= 0 ? boxes[selected] : null
        $('#sl-selinfo').textContent = b
            ? `item ${String(selected + 1).padStart(2, '0')}`
            : 'nenhuma caixa selecionada'
        for (const [input, v] of [
            [selX, b?.x],
            [selY, b?.y],
            [selW, b?.w],
            [selH, b?.h],
    ...
    async function exportZip(): Promise<void> {
        ...
        try {
            ...
            for (let i = 0; i < ordered.length; i++) {
                ...
                files[`${base}_${String(i + 1).padStart(3, '0')}.png`] = new Uint8Array(
                    await blob.arrayBuffer(),
                )
                if (i % 8 === 7) await new Promise((r) => setTimeout(r, 0))
                if (!alive) return
            }
            files[`${base}.json`] = strToU8(
                JSON.stringify(
                    {
                        meta: {
                            app: 'SpriteForge',
