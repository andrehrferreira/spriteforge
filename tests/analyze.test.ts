...
function sample(w: number, h: number, paint: (x: number, y: number) => [number, number, number, number]): FrameSample {
...
describe('detectores', () => {
  ...
  it('detectQuantization propõe 256 sem paleta e 128 com poucas cores', () => {
    ...
    expect(detectQuantization([anim('a', [s])], { ...exportCfg, colors: -1 }, 1000)[0]?.params.cores).toBe(256)
    expect(detectQuantization([anim('a', [s])], { ...exportCfg, colors: 256 }, 1000)[0]?.params.cores).toBe(128)
  })
